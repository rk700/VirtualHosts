...
static const char *binarySearchAddr(const char *hostname) {
    int start = 0, end = hostsLen-1, mid;
    const char *h;
    while(start <= end) {
        mid = start + ((end-start)>>1);
        h = hosts[mid].hostname;
        if(strcmp(h, hostname) < 0) {
            start = mid + 1;
        }
        else if(strcmp(h, hostname) > 0) {
            end = mid -1;
        }
        else {
            return hosts[mid].addr;
        }
    }
    return NULL;
}
...
static int new_getaddrinfo(const char *hostname, const char *servname,
    ...
    if(!(hints->ai_flags & AI_NUMERICHOST)) {
        const char *matchedAddr = binarySearchAddr(hostname);
        if(matchedAddr) {
//            LOGE("find matched addr for %s", hostname);
            ...
            inet_pton(AF_INET, matchedAddr, &(sa.sin_addr));
